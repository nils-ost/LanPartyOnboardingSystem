...
import { Vlan, VlanPurposeType } from '../interfaces/vlan';
...
export class VlanService {
  ...
  public createVlan(number: number, purpose: number, desc: string): Observable<Vlan> {
    let vlan = {'number': number, 'purpose': purpose, 'desc': desc};
    return this.http.post<Vlan>(this.vlanUrl, vlan, {withCredentials:true});
  }

  public updateVlan(id: string, number: number, purpose: number, desc: string): Observable<Vlan> {

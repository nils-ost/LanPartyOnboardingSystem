...
class Table(ElementBase):
    ...
    def validate(self):
        ...
        if seat_pool is None:
            errors['seat_ip_pool_id'] = f"There is no IpPool with id '{self['seat_ip_pool_id']}'"
        elif not docDB.get('VLAN', seat_pool['vlan_id'])['purpose'] == 0:
            errors['seat_ip_pool_id'] = 'VLAN of IpPool needs to be of purpose 0 (play/seats)'
        elif docDB.search_one(self.__class__.__name__, {'add_ip_pool_id': self['seat_ip_pool_id']}) is not None:
            errors['seat_ip_pool_id'] = 'allready in use as add_ip_pool_id on different Table'
        add_pool = docDB.get('IpPool', self['add_ip_pool_id'])
        if add_pool is None:
            errors['add_ip_pool_id'] = f"There is no IpPool with id '{self['add_ip_pool_id']}'"
        elif not docDB.get('VLAN', add_pool['vlan_id'])['purpose'] == 0:
            errors['add_ip_pool_id'] = 'VLAN of IpPool needs to be of purpose 0 (play/seats)'
        elif docDB.search_one(self.__class__.__name__, {'seat_ip_pool_id': self['add_ip_pool_id']}) is not None:
            errors['add_ip_pool_id'] = 'allready in use as seat_ip_pool_id on different Table'

...
class TestSession(unittest.TestCase):
    ...
    def test_user_id_FK_and_notnone(self):
        self.assertEqual(len(Session.all()), 0)
        # user_id can't be None
        el = Session({'till': int(datetime.now().timestamp() + 60), 'ip': '127.0.0.1', 'user_id': None})
        self.assertIn('user_id', el.save()['errors'])
        self.assertEqual(len(Session.all()), 0)
        # user_id can't be a random string
        el = Session({'till': int(datetime.now().timestamp() + 60), 'ip': '127.0.0.1', 'user_id': 'somerandomstring'})
        self.assertIn('user_id', el.save()['errors'])
        self.assertEqual(len(Session.all()), 0)
        # but a valid user_id can be stored
        el = Session({'till': int(datetime.now().timestamp() + 60), 'ip': '127.0.0.1', 'user_id': self.p1_id})
    ...
    def test_till_in_the_future(self):
        self.assertEqual(len(Session.all()), 0)
        # can't be now
        el = Session({'till': int(datetime.now().timestamp()), 'ip': '127.0.0.1', 'user_id': self.p1_id})
        self.assertIn('till', el.save()['errors'])
        self.assertEqual(len(Session.all()), 0)
        # can't be in the past
        el = Session({'till': int(datetime.now().timestamp() - 1), 'ip': '127.0.0.1', 'user_id': self.p1_id})
        self.assertIn('till', el.save()['errors'])
        self.assertEqual(len(Session.all()), 0)
        # can only be in the future
        el = Session({'till': int(datetime.now().timestamp() + 2), 'ip': '127.0.0.1', 'user_id': self.p1_id})
        self.assertNotIn('errors', el.save())
        self.assertEqual(len(Session.all()), 1)
